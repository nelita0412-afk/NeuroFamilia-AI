...
export function LandingFooter() {
  return (
    <footer className="border-t border-white/10 bg-[#0B3B82] py-12 text-[#F8FBFF]">
      <div className="mx-auto flex max-w-6xl flex-col items-center gap-8 px-5 sm:px-8 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex flex-col items-center gap-3 lg:items-start">
          <Link href="/" className="flex items-center gap-3" aria-label="NeuroFamilia Galápagos">
            ...
            <div>
              ...
              <p className="text-[10px] uppercase tracking-[0.2em] text-[#00B8D9]">
        ...
        <nav aria-label="Caminos" className="grid grid-cols-2 gap-x-10 gap-y-2 text-center lg:text-left">
          {PATHS.map((path) => (
            <Link
              key={path.href}
              href={path.href}
              className="text-sm text-[#F8FBFF]/75 transition-colors duration-200 hover:text-[#00B8D9]"
            >
              {path.label}
            </Link>
          ))}
        ...
        <div className="flex flex-col items-center gap-2 lg:items-end">
          <Link href="/login" className="text-sm font-bold text-[#00B8D9] transition-colors duration-200 hover:text-[#F8FBFF]">
            Iniciar sesión
          </Link>
